...
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`
    ...
    CREATE TABLE sdk.sdk_chats (
      id                  UUID PRIMARY KEY DEFAULT make_packed_uuid(0, ${sql.lit(ENTITY_TYPE_MAP.SdkChat)}),
    ...
    ALTER TABLE sdk.sdk_chats ADD CONSTRAINT chk_sdk_chats_entity_type CHECK (extract_entity_type(id) = ${sql.lit(ENTITY_TYPE_MAP.SdkChat)});
    ...
    CREATE TABLE sdk.sdk_signing_keys (
      id                  UUID PRIMARY KEY DEFAULT make_packed_uuid(0, ${sql.lit(ENTITY_TYPE_MAP.SdkSigningKey)}),
    ...
    ALTER TABLE sdk.sdk_signing_keys ADD CONSTRAINT chk_sdk_signing_keys_entity_type CHECK (extract_entity_type(id) = ${sql.lit(ENTITY_TYPE_MAP.SdkSigningKey)});
