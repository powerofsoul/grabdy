...
import type { GitHubPR, MessageList } from '../types';
import { buildContextHeader, buildSyncedItem, extractLabels, formatGitHubDate } from '../utils';

import { GitHubIssueFetcher } from './issue.fetcher';

@Injectable()
export class GitHubPrFetcher {
  private readonly logger = new Logger(GitHubPrFetcher.name);
  ...
  async fetchItem(
    ...
    const comments = await this.issueFetcher.fetchIssueComments(
      octokit,
      owner,
      repoName,
      repoFullName,
