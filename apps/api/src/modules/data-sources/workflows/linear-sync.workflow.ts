...
export async function linearSyncWorkflow(params: {
  orgId: DbId<'Org'>;
  ...
  while (hasMore) {
    if (++page > MAX_SYNC_PAGES) {
      throw new Error(
        `Linear sync exceeded ${MAX_SYNC_PAGES} pages, aborting to prevent infinite loop`
      );
    }
    ...
    if (result.type !== 'items') {
      throw new Error(`Linear sync expected items result, got ${result.type}`);
    }
...
export async function linearWebhookItemWorkflow(params: {
  orgId: DbId<'Org'>;
  connectionId: DbId<'Connection'>;
  event: {
