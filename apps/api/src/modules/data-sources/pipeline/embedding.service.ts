...
export class EmbeddingService {
  ...
  async embedAndStore(
    ...
    for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
      ...
      await this.setProgress(job, dataSourceId, orgId, batchProgress);
    }
  }

  /**
   * Update processing progress in both the database (for the frontend/stale checker)
   * and on the BullMQ job (keeps the worker lock alive across instances).
   */
  async setProgress(
    job: Job,
    dataSourceId: DbId<'DataSource'>,
