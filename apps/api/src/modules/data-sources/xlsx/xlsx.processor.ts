...
import { chunkSheets } from '../chunking/chunk-content';
...
@Processor('ds-xlsx', { concurrency: DS_CONCURRENCY_GLOBAL, lockDuration: DS_LOCK_DURATION_MS })
export class DsXlsxProcessor extends WorkerHost {
  private readonly logger = new Logger(DsXlsxProcessor.name);
  ...
  async process(job: Job): Promise<void> {
    ...
    await this.pipeline.run(job, async () => {
      ...
      if (result.type !== 'sheets') {
        throw new Error('XLSX extractor returned unexpected result type');
      }
      if (!result.text.trim()) {
  ...
  onFailed(job: Job, err: Error) {
    ...
    this.notificationQueue
      .add('slack', {
        ...
        text: `XLSX processing failed: ${job.name}(${job.id}) - ${err.message}`,
      })
