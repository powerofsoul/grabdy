...
export function ChatPanel({ headerSlot, trailingSlot, headerHeight = 48, initialThreadId, onThreadChange }: ChatPanelProps) {
          ...
          {threadManager.isLoadingThreads ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.25, px: 1 }}>
              {threadManager.threads.map((thread) => (
                <Box
                  key={thread.id}
                  onClick={() => { threadManager.loadThread(thread.id); setThreadPanelOpen(false); }}
                  sx={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 1.5,
                    ...
                    borderRadius: 1.5,
                    cursor: 'pointer',
                    bgcolor: thread.id === threadManager.activeThreadId ? alpha(ct, 0.06) : 'transparent',
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography
                        ...
                        sx={{
                          ...
                          color: thread.id === threadManager.activeThreadId ? 'text.primary' : 'text.secondary',
                        }}
                      >
